...
def code_with_ai(username = ""):
    ...
    with output_col:
        ...
        if st.button("Run Code"):
            log_action(username,f"User attempted to run '{language}' code.")
            if language == 'python':
                output = io.StringIO()
                sys.stdout = output 
                ...
                try:
                    ...
                    log_action(username,"'Python' Code ")
                    log_action(username,f"```{st.session_state.code}```")
                    log_action(username,"\n\n'Python' code executed successfully.")
                except Exception as e:
                    sys.stdout = sys.__stdout__  
                    st.error(f"Error in code execution: {e}")
                    log_action(username,f"Error executing Python code: '{e}'")
            else:
                payload = {
                    "language": language,
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        log_action(username,f"'{language}' Code")
                        log_action(username,f"```{st.session_state.code}```")
                        log_action(username,f"Successfully executed '{language}' code via API.")
                    else:
                        st.error(f"Error executing code: {result}")
                        log_action(username,f"API returned an error: '{result}'")
                except Exception as e:
                    st.error(f"Error: {e}")
                    log_action(username,f"Error calling execution API: '{e}'")


    if st.button("Code With AI 🤖"):
            log_action(username,"User requested AI code explanation.")
            genai.configure(api_key=GEMINIAI_API_KEY)
            try:
                model = genai.GenerativeModel(model_name="gemini-1.5-pro")
                response = model.generate_content(f"""Assume you are an expert programmer. Here is the user-provided code:
                Code: {code}
                Language: {language}
                Please explain the code and provide suggestions for improvement.""")
                if response.text:
                    st.markdown(response.text)
                    log_action(username,"AI response generated successfully.")
                else:
                    st.warning("No response from the AI model.")
                    log_action(username,"AI did not return a response.")
            except Exception as e:
                st.error(f"Error using Generative AI: {e}")
                log_action(username,f"Error with AI generation: {e}")

    # if st.button("Code With AI 🤖"):
    #     logging.info("User requested AI code explanation.")
    #     genai.configure(api_key=GEMINIAI_API_KEY)
