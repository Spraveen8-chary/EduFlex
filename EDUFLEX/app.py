...
from helpers.quiz import get_quiz_data, get_video_text
...
if chosen == "Learn":
    ...
    def youtube(url):
        try:
            ...
            return "Title unavailable", "Thumbnail unavailable"
        except PytubeError as e:
            return "Error fetching video data", "No thumbnail"

    titles = []
    locations=[]
    thumbnails = []

    placeholder_thumbnail = "https://via.placeholder.com/720x405.png?text=No+Thumbnail"

    for video_url in links:
        video_title , video_thumbnail = youtube(video_url)
        if video_thumbnail == "No thumbnail":
            video_thumbnail = placeholder_thumbnail 
